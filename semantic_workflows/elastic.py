"""
Core elastic properties workflow functions.

These functions contain the computational logic for elastic constant calculations
and mechanical testing, without framework-specific decorators.
"""
...
def _displace(lmp, dir, pair_style, pair_coeff):
    """Helper function for elastic constant displacement calculations."""
    if dir == 1:
...
def calculate_elastic_constants(
    ...
    """
    Calculate elastic constants using finite deformation method.
    ...
    cores : int, optional
        Number of cores for LAMMPS
    finite_deformation_size : float, optional
        Size of finite deformation
    ...
    potential_doi : str, optional
        DOI of potential (pyiron only)

    Returns
    -------
    dict
        Dictionary with elastic constants and moduli
    """
        ...
        outputs = [
            {
                ...
                "value": c_matrix[0, 0],
                "unit": "GigaPA",
                "associate_to_sample": [sample_id],
                "basename": "ElasticConstant",
            },
            {
                "label": "C12",
                "value": c_matrix[0, 1],
...
def mechanical_response_test(
    ...
    """
    Perform mechanical response test (compression or tensile).
    ...
    pair_coeff : str
        LAMMPS pair coefficients
    mode : str, optional
        Test mode: "hydrostatic" or "uniaxial"
    cores : int, optional
        Number of cores for LAMMPS
    ...
    # ------------------------
    lmp.command(
        f"velocity all create {temperature} {np.random.randint(10000)} mom yes rot yes dist gaussian"
    )
    ...
    for T1, T2 in [
        ...
        lmp.command(
            f"fix 3 all nph x 0.0 0.0 0.1 y 0.0 0.0 0.1 z 0.0 0.0 0.1 drag 1.0 nreset 1000"
        )
    ...
    if mode == "hydrostatic":
        ...
        lmp.command(f"fix def all deform 1 z erate {strain_rate} units box remap x")
