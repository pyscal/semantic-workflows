...
def indentation_test(
    ...
    # Equilibration phase
    lmp.command(
        f"velocity all create {initial_temperature} {np.random.randint(1000, 9999999)} mom yes rot yes dist gaussian"
    )

    # Step 1: Heat to annealing temperature
    lmp.command(
        f"fix 2 all temp/rescale 1 {initial_temperature} {annealing_temperature} 1.0 1.0"
    )
    lmp.command(
        f"fix 3 all nph x 0.0 0.0 {ts*1000} y 0.0 0.0 {ts*1000} z 0.0 0.0 {ts*1000} drag 1.0 nreset 1000"
    )
    ...
    lmp.command(
        f"fix 2 all temp/rescale 1 {annealing_temperature} {annealing_temperature} 1.0 1.0"
    )
    lmp.command(
        f"fix 3 all nph x 0.0 0.0 {ts*1000} y 0.0 0.0 {ts*1000} z 0.0 0.0 {ts*1000} drag 1.0 nreset 1000"
    )
    ...
    lmp.command(
        f"fix 2 all temp/rescale 1 {annealing_temperature} {initial_temperature} 1.0 1.0"
    )
    lmp.command(
        f"fix 3 all nph x 0.0 0.0 {ts*1000} y 0.0 0.0 {ts*1000} z 0.0 0.0 {ts*1000} drag 1.0 nreset 1000"
    )
    ...
    lmp.command(
        f"fix Therm b_the nvt temp {initial_temperature} {initial_temperature} {ts*100}"
    )
    ...
    # Apply indenter
    lmp.command(
        f"fix indent all indent 10.0 sphere v_Ind_x v_Ind_y v_Ind_z {indenter_radius} units box"
    )
    ...
    # Dump output
    lmp.command(
        f"dump OUT1 all custom {dump_interval} Indent.* id type x y z vx vy vz c_Ene c_Str[1] c_Str[2] c_Str[3] c_Str[4] c_Str[5] c_Str[6] c_f[1] c_f[2] c_f[3]"
    )
    ...
    lmp.command(
        f'fix def1 all print {print_interval} "$(step), $(temp), ${{Ind_x}}, ${{Ind_y}}, ${{Ind_z}}, ${{f0}}, ${{f1}}, ${{f2}}, ${{f3}}" append Result-indent.txt screen no'
    )
    ...
    lmp.command(
        "dump OUT1 all custom 1 Indent.* id type x y z vx vy vz c_Ene c_Str[1] c_Str[2] c_Str[3] c_Str[4] c_Str[5] c_Str[6] c_f[1] c_f[2] c_f[3]"
    )
    lmp.command("run 1")
    ...
    results_data = pd.read_csv(
        "Result-indent.txt",
        names=["step", "temp", "Ind_x", "Ind_y", "Ind_z", "f0", "f1", "f2", "f3"],
    )
...
def plot_force_depth(results):
    ...
    ax1.plot(
        results["indentation_depth"], results["indentation_force"], "b-", linewidth=2
    )
